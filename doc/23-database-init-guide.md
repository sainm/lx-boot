...
- 应用回滚：回退到兼容旧 API 的上一构建；V2-V16 主要增加或放宽结构，V17/V18 则要求所有直接租户写入显式携带 `tenant_id`。V19 后旧应用不认识导出 `DEAD_LETTER`，V20 后旧通知 Worker 又无法写入处理租约，V22 后旧应用不会写质量留痕，V23 后旧应用不会写评分轨迹但可继续读取既有结果；回滚前必须暂停两类 Worker，并确认没有 `PROCESSING`/`DEAD_LETTER` 遗留任务或投递。当前上一版本代码路径已由完整回归和开发种子验证会写租户，但任何更旧制品必须先做兼容性测试；不满足时不得在 V18 后回滚。回退期间不得触发重新评分；V11 回退应用后暂停高风险结案；V12/V13/V16/V22/V23 回退应用后停止编辑治理证据、质量策略、评分审计和量表发布。已验证的外键、`NOT NULL`、V14 索引、V15 类型、V16 新结构以及 V22/V23 新列优先保留，不在故障窗口做破坏性降级。
