...
- `V22__assessment_quality_outcomes.sql`：为答卷和结果增加质量状态、问题码、缺失比例和作答时长留痕；历史行保持 `NOT_EVALUATED`。
- `V23__scoring_trace_audit.sql`：为评分结果增加可空 JSONB 审计轨迹，记录逐题中间分、维度聚合、质量处理、常模选择和规则匹配，不保存自由文本答案。
- `V24__scale_skip_rules.sql`：为量表增加声明式跳题规则 JSONB；跳题不产生答案，也不进入评分。
- `V25__scale_publication_review_evidence.sql`：为发布评审增加评审人姓名快照、资质引用、证据引用和评审范围，并对可确定的历史评审人姓名做一次性回填。
- `V26__enforce_scale_publication_review_evidence.sql`：以 `NOT VALID` CHECK 阻止新的无审批人、无证据、无评审范围的 `APPROVED` 记录；历史行保留但后续更新同样受约束。
- `V27__scale_time_question_type.sql`：将受控题型白名单显式扩展 `TIME`，支持睡眠/时间类声明式重编码，不引入新真实量表。
- `V28__safety_response_policy_professional_review.sql`：为安全响应策略增加专业复核时间戳，并约束审批人与专业复核人分离后才能激活。
...
| V22 | 答卷/结果增加质量状态、问题码、缺失比例和作答时长留痕；已有行保持 `NOT_EVALUATED` | 仅新增列和质量值域约束；不会扫描或改写历史答案内容 | 低峰执行并确认短时元数据锁；失败时前滚修复，禁止删除 V22 列 | 旧应用继续读既有结果；新应用只有在 V22 后才写入质量留痕 |
| V23 | 结果增加可选 `scoring_trace_json` 评分审计轨迹 | 仅新增可空 JSONB 和对象类型约束；不写入自由文本答案 | 低峰执行并确认短时元数据锁；失败时前滚修复，禁止删除 V23 列 | 旧应用忽略该列；新应用保存逐题原始/反向/加权、维度、质量、常模和规则证据 |
| V24 | 量表增加声明式 `skip_rules_json` | 仅新增可空 JSONB 列；不扫描或改写历史题目 | 低峰执行；旧应用忽略跳题规则，新任务读取时对畸形规则失败关闭 |
| V25 | 发布评审增加评审人姓名快照、资质、证据引用和评审范围，并回填可确定姓名 | 加列为元数据变更；姓名回填按 `sys_user` 关联更新历史评审行 | 迁移前备份；已有评审不被伪造为已提供专业资质；新审批必须补齐证据 |
| V26 | 以 `NOT VALID` CHECK 约束新的 `APPROVED` 发布评审证据 | `NOT VALID` 不扫描历史行，但立即约束新写入和更新 | 与 V25 同窗口部署；回退应用时必须暂停量表发布，不能绕过约束 |
| V27 | 重建题型白名单并加入 `TIME` | 仅重建 CHECK；历史题目值均在白名单内 | 低峰执行；旧应用遇到 `TIME` 题目必须拒绝而不是降级为文本 |
| V28 | 安全响应策略增加专业复核时间和双人分离激活约束 | 加列 + `NOT VALID` CHECK；历史策略保持原状态 | 与应用同步部署；回退时暂停安全策略审批与激活 |

V2/V3 的 `NOT VALID` 约束会立即拒绝新的非法数据，但不会在部署事务中扫描全部历史行。历史数据清理完成后，应新增独立迁移执行 `VALIDATE CONSTRAINT`，不要回改 V2/V3。

## 5. 回滚

- 应用回滚：回退到兼容旧 API 的上一构建；V2-V16 主要增加或放宽结构，V17/V18 则要求所有直接租户写入显式携带 `tenant_id`。V19 后旧应用不认识导出 `DEAD_LETTER`，V20 后旧通知 Worker 无法写入处理租约，V22 后旧应用不会写质量留痕，V23 后旧应用不会写评分轨迹但可继续读取既有结果，V24/V25/V26 后旧应用不会生成跳题规则或完整评审证据，V27 后旧应用不认识 `TIME` 题型，V28 后旧应用无法完成带专业复核时间的安全策略激活；回滚前必须暂停两类 Worker，并确认没有 `PROCESSING`/`DEAD_LETTER` 遗留任务或投递。当前上一版本代码路径已由完整回归和开发种子验证会写租户，但任何更旧制品必须先做兼容性测试；不满足时不得在 V18 后回滚。回退期间不得触发重新评分；V11 回退应用后暂停高风险结案；V12/V13/V16/V22/V23/V24-V28 回退应用后停止编辑治理证据、质量策略、评分审计、跳题规则、发布评审和安全策略审批。已验证的外键、`NOT NULL`、V14 索引、V15 类型、V16 新结构以及 V22-V28 新列优先保留，不在故障窗口做破坏性降级。
...
GitHub Actions 使用 PostgreSQL 17.6，并执行 16 个隔离 PostgreSQL Case，其中包括：
