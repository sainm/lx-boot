...
- 当前进展（2026-04-16）：已补充导出产物存储抽象，支持 `LOCAL_PATH / KEYED_PATH / S3_URI_COMPAT / HTTP_OBJECT_STORAGE` 多种模式；其中 `HTTP_OBJECT_STORAGE` 可通过 HTTP 网关落地对象存储语义，导出任务状态接口与 Admin Web 已可展示 `storageLocation`
- 当前进展（2026-04-16）：已补充 `claimPending` / `claimPendingJobs` 原子抢占、后台 `ExportJobWorker` 自动补跑、`PROCESSING` 超时恢复，以及导出中心 Admin Web 值守面板，可覆盖“提交即执行 + 后台捞漏 + 管理员重试”的多实例治理第一版
- 当前仍未完成真正的厂商对象存储 SDK 直连，以及更细粒度的实例标识、重试次数和跨实例调度可观测性
...
- `notification`
  ...
  - 通知投递运维：
    - 后端已有投递明细、汇总、重试接口，并新增全局通知运维检索与批量重试接口
    - 前端已补齐概况、通知级明细查询、失败重试、全局筛选、失败聚合与批量操作入口
    - 仍可继续增强独立运维工作台的审计留痕和更复杂的模板/策略联动
  - 深链上下文：
...
- `export`
  - 导出文件持久化：
    - 已支持 DB 元数据 + 本地文件落盘
    - 已支持对象存储语义抽象与 HTTP 对象存储网关适配
    - 尚未对接真实厂商对象存储 SDK
  - 多实例导出治理：
    - 已有调度锁、任务抢占、后台补跑与超时恢复基础
    - 已形成第一版跨实例任务抢占与自动重放调度闭环
    - 仍可继续补实例标识、调度来源和重试次数等治理字段
- `warning`
  - 预警升级与催办：
    - 第一版规则已落地
