...
`scripts/run-backup-restore-rehearsal.sh` 只操作名称严格匹配 `psy_recovery_source_*` 和 `psy_recovery_restore_*` 的隔离数据库，不连接、迁移、清理或重建用户现有 `lx/public`。演练使用与应用相同的 Flyway V1-V28 和开发技术种子；技术种子不代表正式量表、授权、常模或临床内容。下方 2026-08-11 的数字是 V23 时代的实测记录，当前结构已扩展到 V28；重新演练时必须以脚本输出的迁移数和 manifest 为准。
...
- 源库和恢复库均应为 28 条成功 Flyway 迁移、3 个任务/答卷/结果/报告、2 个预警、1 个干预、1 条安全审计和 3 个数据库内导出文件；V17/V18 的租户约束、V19 导出租约、V20 通知投递租约、V21 历史游标索引、V22 质量留痕结构、V23 评分轨迹 JSONB 约束、V24 跳题规则、V25/V26 发布评审证据约束、V27 `TIME` 题型白名单及 V28 安全策略专业复核结构均包含在 manifest 对比中。
