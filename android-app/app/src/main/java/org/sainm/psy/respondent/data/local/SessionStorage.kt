...
class SessionStorage(context: Context) {
    ...
    fun readTokens(): SessionTokens? {
        ...
        val decoded = runCatching {
            val jsonValue = if (raw.startsWith(ENCRYPTED_PREFIX)) decrypt(raw.removePrefix(ENCRYPTED_PREFIX)) else raw
    ...
    fun writeTokens(tokens: SessionTokens) {
        val encoded = encrypt(json.encodeToString(SessionTokens.serializer(), tokens))
        preferences.edit().putString(KEY_TOKENS, ENCRYPTED_PREFIX + encoded).apply()
    }
    ...
    fun readLocaleTag(): String {
        val stored = preferences.getString(KEY_LOCALE_TAG, null)
        if (stored in SUPPORTED_LOCALES) return requireNotNull(stored)
    ...
    fun writeLocaleTag(localeTag: String) {
        require(localeTag in SUPPORTED_LOCALES) { "Unsupported locale tag" }
        preferences.edit().putString(KEY_LOCALE_TAG, localeTag).apply()
    }
    ...
    private companion object {
        ...
        val SUPPORTED_LOCALES = setOf("zh-CN", "ja-JP", "en-US")
    }
}
