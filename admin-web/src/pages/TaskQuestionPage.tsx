...
export function TaskQuestionPage() {
  ...
  const answeredCount = Form.useWatch([], form)
    ? toAnswerItems(questions, form.getFieldsValue()).length
    : 0;
  ...
  const currentAnswerValue = currentQuestion ? Form.useWatch(`question-${currentQuestion.questionId}`, form) : undefined;
  const currentAnswerText = currentQuestion ? Form.useWatch(`question-${currentQuestion.questionId}-text`, form) : undefined;
  ...
  const handleNext = async () => {
    ...
    await form.validateFields([`question-${currentQuestion.questionId}`]);
