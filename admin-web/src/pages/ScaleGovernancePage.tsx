...
import { governanceOptionLabel } from "../i18n/enumLabel";
...
/** Governance selects show business labels instead of raw backend codes. */
function localizedOptions(t: (key: string) => string, values: readonly string[]) {
  return values.map((value) => ({ value, label: governanceOptionLabel(t, value) }));
}

const reviewOptions = (t: (key: string) => string) => localizedOptions(t, REVIEW_STATUSES);
const copyrightOptions = (t: (key: string) => string) =>
  localizedOptions(t, ["PENDING_REVIEW", "AUTHORIZED", "PUBLIC_DOMAIN", "RESTRICTED", "EXPIRED", "REJECTED"]);
const authorizationOptions = (t: (key: string) => string) =>
  localizedOptions(t, ["PENDING_REVIEW", "AUTHORIZED", "NOT_REQUIRED", "RESTRICTED", "EXPIRED", "REJECTED"]);
  ...
  return (
    <Card size="small">
      <Row gutter={16}>
        ...
        <SelectField name="copyrightStatus" labelKey="scaleGovernance.copyrightStatus" options={copyrightOptions(t)} required />
        <TextField name="rightsHolder" labelKey="scaleGovernance.rightsHolder" />
        <SelectField name="authorizationStatus" labelKey="scaleGovernance.authorizationStatus" options={authorizationOptions(t)} required />
        ...
        <Col xs={24} md={12}><Form.Item name={["governance", "governanceStatus"]} label={t("scaleGovernance.reviewStatus")} rules={[{ required: true }]}><Select options={reviewOptions(t)} /></Form.Item></Col>
      </Row>
    </Card>
  );
}
    ...
    <Form.List name="translations">
      {(fields) => <Space direction="vertical" size={12} style={{ width: "100%" }}>{fields.map((field) => (
        <Card key={field.key} size="small" title={<Form.Item noStyle shouldUpdate>{({ getFieldValue }) => getFieldValue(["translations", field.name, "localeCode"])}</Form.Item>}>
          ...
          <Form.Item name={[field.name, "reviewStatus"]} label={t("scaleGovernance.reviewStatus")} rules={[{ required: true }]}><Select options={reviewOptions(t)} /></Form.Item>
        </Card>
      ))}</Space>}
    </Form.List>
  );
...
function QualityFields() {
  ...
  return <Space direction="vertical" size={12} style={{ width: "100%" }}>
    <Card size="small" title={t("scaleGovernance.qualityPolicy")}>
      <Row gutter={16}>
        <Col xs={24} md={12}><Form.Item name={["qualityPolicy", "missingAnswerPolicy"]} label={t("scaleGovernance.missingPolicy")} rules={[{ required: true }]}><Select options={localizedOptions(t, ["REJECT", "ALLOW", "PRORATE", "PENDING_PROFESSIONAL_REVIEW"])} /></Form.Item></Col>
        ...
        <Col xs={24} md={12}><Form.Item name={["qualityPolicy", "invalidResultAction"]} label={t("scaleGovernance.invalidAction")} rules={[{ required: true }]}><Select options={localizedOptions(t, ["INVALIDATE", "REQUIRE_REVIEW", "ALLOW_WITH_WARNING"])} /></Form.Item></Col>
    ...
    <Form.List name="validityRules">{(fields, { add, remove }) => <Card size="small" title={t("scaleGovernance.validityRules")} extra={<Button icon={<PlusOutlined />} onClick={() => add({ ruleCode: "", ruleType: "CONSISTENCY", ruleVersion: "1", configJson: "{}", reviewStatus: "DRAFT", enabled: false, sortNo: fields.length })}>{t("scaleGovernance.addRule")}</Button>}>
      <Space direction="vertical" size={8} style={{ width: "100%" }}>{fields.map((field) => <Card key={field.key} size="small" extra={<Button danger type="text" icon={<DeleteOutlined />} onClick={() => remove(field.name)} />}>
        <Row gutter={12}>
          ...
          <Col xs={24} md={8}><Form.Item name={[field.name, "ruleType"]} label={t("scaleGovernance.ruleType")} rules={[{ required: true }]}><Select options={localizedOptions(t, ["CONSISTENCY", "CONTRADICTION", "DURATION", "RESPONSE_PATTERN", "CUSTOM_EXTENSION"])} /></Form.Item></Col>
          ...
          <Col xs={24} md={8}><Form.Item name={[field.name, "reviewStatus"]} label={t("scaleGovernance.reviewStatus")}><Select options={reviewOptions(t)} /></Form.Item></Col>
...
function AlgorithmFields() {
  ...
  return <Card size="small"><Row gutter={16}>
    ...
    <Col xs={24} md={12}><Form.Item name={["algorithmBinding", "implementationType"]} label={t("scaleGovernance.implementationType")} rules={[{ required: true }]}><Select options={localizedOptions(t, ["BUILTIN", "RESTRICTED_EXTENSION"])} /></Form.Item></Col>
    ...
    <Col xs={24} md={12}><Form.Item name={["algorithmBinding", "reviewStatus"]} label={t("scaleGovernance.reviewStatus")} rules={[{ required: true }]}><Select options={reviewOptions(t)} /></Form.Item></Col>
  </Row></Card>;
}
  ...
  return <Form.List name="normGovernance">{(fields) => <Space direction="vertical" size={8} style={{ width: "100%" }}>{fields.map((field) => {
    ...
    return <Card key={field.key} size="small" title={labels.get(normId) ?? `#${normId}`}>
      ...
      <Row gutter={12}>
        ...
        <Col xs={24} md={8}><Form.Item name={[field.name, "reviewStatus"]} label={t("scaleGovernance.reviewStatus")}><Select options={localizedOptions(t, ["PENDING_REVIEW", "APPROVED", "REJECTED", "EXPIRED"])} /></Form.Item></Col>
      </Row>
    </Card>;
  })}</Space>}</Form.List>;
}
